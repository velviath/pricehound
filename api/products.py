...
_DEMO_PRODUCTS = [
    ...
    {
        "url": "https://www.amazon.com/dp/B09TMF6741",
        "name": "Kindle Paperwhite (16 GB) — Now with 3 months Kindle Unlimited",
        "current_price": 129.99,
        "image_url": "/static/img/kindle-paperwhite.jpg",
        "source": "Amazon",
        "history": [159.99, 149.99, 145.00, 139.99, 134.99, 129.99],
    },
]


async def seed_demo_products() -> None:
    """Create the 3 demo products on startup if they don't already exist."""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            now = datetime.utcnow()
            for demo in _DEMO_PRODUCTS:
                if await q.get_product_by_url(conn, demo["url"]):
                    continue
                ...
                # Insert 6 months of synthetic price history (one point per month)
                for i, price in enumerate(demo["history"]):
